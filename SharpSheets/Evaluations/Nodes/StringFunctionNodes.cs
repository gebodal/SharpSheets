...
namespace SharpSheets.Evaluations.Nodes {
	...
	public abstract class StringConvertFunction : AbstractSingleArgFunction {
		...
		protected override EnvironmentFunctionArg GetArgument(EvaluationContext context) {
			return new EnvironmentFunctionArg("text", context.GetType<StringEvaluationType>(), null);
		}
		...
		public sealed override EvaluationType GetReturnType(EvaluationContext context, EvaluationNode arg) {
			...
			return StringEvaluationType.IsString(argType) ? context.GetType<StringEvaluationType>() : throw new EvaluationTypeException($"{Name} not defined for value of type {argType}.");
		}
		...
		public sealed override EvaluationValue Evaluate(IEnvironment environment, EvaluationNode arg) {
			...
			if (StringEvaluationType.TryGetString(a, out string? aString)) {
				return new EvaluationValue(ConvertString(aString), environment.GetType<StringEvaluationType>());
			}
			else {
		...
		*/
		...
		public override EnvironmentFunctionArguments GetArguments(EvaluationContext context) {
			return new EnvironmentFunctionArguments(null,
				new EnvironmentFunctionArgList(
					new EnvironmentFunctionArg("separator", context.GetType<StringEvaluationType>(), null),
					new EnvironmentFunctionArg("arrayOrTuple", null, null)
					)
			);
		}
			...
			if (StringEvaluationType.TryGetString(a, out string? separator)) {
				...
				foreach(EvaluationValue item in b.Type.Iteration(b) ?? throw new EvaluationTypeException($"Cannot iterate through value of type {b.Type}.")) {
					if(StringEvaluationType.TryGetString(item, out string? itemStr)) {
						...
						throw new EvaluationTypeException($"Cannot convert item of type {item.Type} to string.");
					}
				}

				return new EvaluationValue(string.Join(separator, items), environment.GetType<StringEvaluationType>());
			}
			else {
		...
		*/
		...
		public override EvaluationType GetReturnType(EvaluationContext context, EvaluationNode[] args) {
			...
			if (StringEvaluationType.IsString(arg1Type) && StringEvaluationType.IsString(arg2Type)) {
				return context.GetType<StringEvaluationType>().MakeArray();
			}
			else {
		...
		public override EvaluationValue Evaluate(IEnvironment environment, EvaluationNode[] args) {
			...
			if (StringEvaluationType.TryGetString(a, out string? text) && StringEvaluationType.TryGetString(b, out string? delimiter)) {
				EvaluationType stringType = environment.GetType<StringEvaluationType>();
				EvaluationValue[] values = text.Split(delimiter).Select(s => new EvaluationValue(s, stringType)).ToArray();
				return ArrayEvaluationType.MakeArray(stringType, values);
			}
			else {
		...
		*/
		...
		public override EnvironmentFunctionArguments GetArguments(EvaluationContext context) {
			return new EnvironmentFunctionArguments(
				"Format must have a string format argument and at least one content argument.",
				new EnvironmentFunctionArgList(new EnvironmentFunctionArg[] {
						new EnvironmentFunctionArg("format", context.GetType<StringEvaluationType>(), null),
						new EnvironmentFunctionArg("content", null, null)
					}, true)
			);
		}
		...
		public override EvaluationType GetReturnType(EvaluationContext context, EvaluationNode[] args) {
			EvaluationType[] returnTypes = args.Select(a => a.GetReturnType()).ToArray();
		...
		public override EvaluationValue Evaluate(IEnvironment environment, EvaluationNode[] args) {
			...
			for (int i = 0; i < contents.Length; i++) {
				...
				// TODO These distinctions aren't necessary anymore, right?
			...
			if (StringEvaluationType.TryGetString(a, out string? format)) {
				try {
					...
					return new EvaluationValue(string.Format(format, contents), environment.GetType<StringEvaluationType>());
				}
	...
	public class StringReplaceFunction : AbstractFunction {
		...
		*/
		...
		public override EnvironmentFunctionArguments GetArguments(EvaluationContext context) {
			EvaluationType stringType = context.GetType<StringEvaluationType>();

			return new EnvironmentFunctionArguments(null,
				new EnvironmentFunctionArgList(
					new EnvironmentFunctionArg("input", stringType, null),
					new EnvironmentFunctionArg("oldValue", stringType, null),
					new EnvironmentFunctionArg("newValue", stringType, null)
					)
			);
		}
		...
		public override EvaluationValue Evaluate(IEnvironment environment, EvaluationNode[] args) {
			...
			if (StringEvaluationType.TryGetString(a, out string? input) && StringEvaluationType.TryGetString(b, out string? oldValue) && StringEvaluationType.TryGetString(c, out string? newValue)) {
				return new EvaluationValue(input.Replace(oldValue, newValue), environment.GetType<StringEvaluationType>());
			}
			else {
