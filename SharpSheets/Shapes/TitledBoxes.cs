...
namespace SharpSheets.Shapes {
	...
	/*
	public class BlockTitledBoxOld : TitledBoxBase {
		...
		public BlockTitledBoxOld(float aspect, string name = "NAME", TextFormat format = TextFormat.REGULAR, float fontSize = 8f, float spacing = 2f, (float x, float y) offset = default, float? headerSize = null, Direction direction = Direction.NORTH) : base(aspect, name, format, fontSize) {
		...
		protected void GetRects(ISharpGraphicsState graphicsState, Rectangle rect, out Rectangle? headerRect, out Rectangle? bodyRect) {
			if (direction == Direction.NORTH) {
				headerRect = Divisions.Row(rect, Dimension.FromPoints(headerSize ?? fontSize * parts.Length + 2 * spacing), 0f, out bodyRect, out _, false, Arrangement.FRONT, LayoutOrder.FORWARD);
			}
			else if (direction == Direction.SOUTH) {
				headerRect = Divisions.Row(rect, Dimension.FromPoints(headerSize ?? fontSize * parts.Length + 2 * spacing), 0f, out bodyRect, out _, false, Arrangement.FRONT, LayoutOrder.BACKWARD);
			}
			else if (direction == Direction.WEST) {
				float width = parts.Select(p => graphicsState.GetWidth(p, format, fontSize)).Max();
				headerRect = Divisions.Column(rect, Dimension.FromPoints(headerSize ?? width + 2 * spacing), 0f, out bodyRect, out _, false, Arrangement.FRONT, LayoutOrder.FORWARD);
			}
			else { // direction == Direction.EAST
				float width = parts.Select(p => graphicsState.GetWidth(p, format, fontSize)).Max();
				headerRect = Divisions.Column(rect, Dimension.FromPoints(headerSize ?? width + 2 * spacing), 0f, out bodyRect, out _, false, Arrangement.FRONT, LayoutOrder.BACKWARD);
			}
		}
		...
		protected override void DrawFrame(ISharpCanvas canvas, Rectangle rect) {
			...
			GetRects(canvas, rect, out Rectangle? headerRect, out _);

			canvas.SetFillColor(canvas.GetBackgroundColor()).SetStrokeColor(canvas.GetForegroundColor());
			canvas.Rectangle(rect).Stroke();

			if (headerRect is not null) {
				canvas.SetFillColor(canvas.GetForegroundColor()).SetStrokeColor(canvas.GetForegroundColor());
				canvas.Rectangle(headerRect).FillStroke();

				canvas.SetTextColor(canvas.GetBackgroundColor());
				canvas.DrawRichText(headerRect, parts.Select(p => new RichString(p, format)).ToArray(), fontSize, new ParagraphSpecification(1.0f, 0f, 0f, 0f), Justification.CENTRE, Alignment.CENTRE, TextHeightStrategy.AscentDescent, false, offset);
			}
			else {
				canvas.LogError(this, $"No space for {nameof(BlockTitledBox)} header.");
			}
		...
		protected override Rectangle GetRemainingRect(ISharpGraphicsState graphicsState, Rectangle rect) {
			GetRects(graphicsState, rect, out _, out Rectangle? bodyRect);
			if (bodyRect is null) { throw new InvalidRectangleException($"Invalid remaining area for {this.GetType().Name}."); }
			return bodyRect.Margins(graphicsState.GetLineWidth() / 2, false);
		}

		public override Rectangle FullRect(ISharpGraphicsState graphicsState, Rectangle rect) {
			if (direction == Direction.NORTH) {
				float headerHeight = headerSize ?? fontSize * parts.Length + 2 * spacing;
				return rect.Margins(graphicsState.GetLineWidth() / 2, true).Margins(headerHeight, 0f, 0f, 0f, true);
			}
			else if (direction == Direction.SOUTH) {
				float headerHeight = headerSize ?? fontSize * parts.Length + 2 * spacing;
				return rect.Margins(graphicsState.GetLineWidth() / 2, true).Margins(0f, 0f, headerHeight, 0f, true);
			}
			else if (direction == Direction.WEST) {
				float headerWidth = headerSize ?? parts.Select(p => graphicsState.GetWidth(p, format, fontSize)).Max() + 2 * spacing;
				return rect.Margins(graphicsState.GetLineWidth() / 2, true).Margins(0f, 0f, 0f, headerWidth, true);
			}
			else { // direction == Direction.EAST
				float headerWidth = headerSize ?? parts.Select(p => graphicsState.GetWidth(p, format, fontSize)).Max() + 2 * spacing;
				return rect.Margins(graphicsState.GetLineWidth() / 2, true).Margins(0f, headerWidth, 0f, 0f, true);
			}
		}
	}
	*/
